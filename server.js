...
// CONFIG
// ─────────────────────────────────────────────
const GEMINI_API_KEY = 'AIzaSyBgIBQwVqQAVTRBXpjXaCN3QACsWK8R3Qo'
...
- You keep a space/cosmic theme in your personality (this is a space-themed chatbot)
...
Memory stats: ${JSON.stringify(this.memory.getStats())}`;
  ...
  async chat(sessionId, userMessage) {
    ...
    this.memory.add({ sessionId, role: 'user', text: userMessage });
    ...
    const recentMsgs = this.sessions.getRecentMessages(sessionId, 12);
    const systemPrompt = this.buildSystemPrompt(userMessage, sessionId);
    ...
    this.memory.add({ sessionId, role: 'assistant', text: responseText });
    this.sessions.addMessage(sessionId, 'assistant', responseText);

    // Self-learning: if message contains a question-answer pair signal
    if (userMessage.includes('?') && responseText.length > 20) {
      this.knowledge.learn(userMessage, responseText, 0.8);
    }

    return {
      response: responseText,
      memoryUsed: this.memory.retrieve(userMessage, 3).length > 0,
      knowledgeUsed: this.knowledge.retrieve(userMessage, 1).length > 0,
      memoryStats: this.memory.getStats()
    };
  }

  // Self-learning from user feedback/correction
  async learnFromFeedback(sessionId, question, correction) {
    this.knowledge.learn(question, correction, 1.0);
    this.memory.add({
      sessionId,
      role: 'fact',
    ...
    feedbackRepo.add({ sessionId, question, correction });
    return { learned: true };
  }
}
